...
def process_csv(input_file_path, output_file_path):
    ...
    for islem_no in df['ISLEM_NO'].unique():
        ...
        # Iterate through unique features (TETKIK_ADI or ALT_TETKIK_ADI)
        for feature_name in pd.concat([patient_data['TETKIK_ADI'], patient_data['ALT_TETKIK_ADI']]).dropna().unique():
            # Extract values for the feature
            feature_data = patient_data[(patient_data['TETKIK_ADI'] == feature_name) | (patient_data['ALT_TETKIK_ADI'] == feature_name)]
...
input_file_path =  "" # ADD INPUT CSV PATH
output_file_path = "" # ADD INPUT CSV PATH